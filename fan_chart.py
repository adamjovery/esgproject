...
def plot_esg_fan_charts(results, T, save_path=None):
    ...
    fan_chart(results["rates"], T, axes[0], "Short-term Interest Rate (Vasicek)", "Interest rate", as_percent=True)
    fan_chart(results["inflation"], T, axes[1], "Inflation (Ornstein-Uhlenbeck)", "Inflation rate", as_percent=True)
