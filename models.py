...
import numpy as np
#Function which simulates the short-term interest rate path under the Vasicek model
#This model assumes short term interest rates follow a mean reverting stochastic process
def vasicek(r0, a, b, sigma, T, steps, paths, z):
    dt = T / steps

    #Intialising matrix of short-term interest rates
    rates = np.zeros((steps + 1, paths))
    rates[0] = r0

    #Charting path of short-term interest rates
    for t in range(1, steps + 1):
        rates[t] = (rates[t-1] + a * (b - rates[t-1]) * dt + sigma * np.sqrt(dt) * z[t-1])
    
    #Returning the path of the short-term interest rates
    return rates
